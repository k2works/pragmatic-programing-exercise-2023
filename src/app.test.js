...
describe("商店データベース", () => {
  ...
  describe("第8章 複数テーブルの結合", () => {
    ...
    test("61:注文番号「202201130115」について、注文番号、注文枝番、商品コード、商品名、数量の一覧を注文番号および注文枝番の順に抽出する。商品名は商品テーブルより取得すること。", async () => {
      const expected = await prisma.$queryRaw`SELECT T.注文番号,T.注文枝番,T.商品コード,S.商品名,T.数量 FROM 注文 AS T JOIN 商品 AS S ON T.商品コード = S.商品コード WHERE T.注文番号 = '202201130115' ORDER BY T.注文番号,T.注文枝番`;
      console.table(expected);
    ...
    test("62:廃番となった商品コード「A0009」について、廃番日より後に注文された注文情報（注文日、注文番号、注文枝番、数量、注文金額）を抽出する。注文金額は単価と数量より算出すること。", async () => {
      const expected = await prisma.$queryRaw`SELECT T.注文日,T.注文番号,T.注文枝番,T.数量,T.数量 * H.単価 AS 注文金額 FROM 注文 AS T JOIN 廃番商品 AS H ON T.商品コード = H.商品コード WHERE T.商品コード = 'A0009' AND T.注文日 >= H.廃番日`;
      console.table(expected);
    ...
    test("63:商品コード「S0604」について、商品情報（商品コード、商品名、単価）とこれまでの注文情報（注文日、注文番号、数量）、さらに単価と数量から売上金額を求め、一覧として抽出する。一覧は、注文のあった順に表示すること。", async () => {
      const expected = await prisma.$queryRaw`SELECT S.商品コード,S.商品名,S.単価,T.注文日,T.注文番号,T.数量,T.数量 * S.単価 AS 売上金額 FROM 商品 AS S JOIN 注文 AS T ON S.商品コード = T.商品コード WHERE S.商品コード = 'S0604' ORDER BY T.注文番号`;
      console.table(expected);
    ...
    test("64:2020年8月に注文のあった商品コードの一覧を抽出する。一覧には、商品名も表示する必要がある。すでに廃番となっている商品に関しては特に考慮しなくてよい（一覧に含まれなくてよい）。", async () => {
      const expected = await prisma.$queryRaw`SELECT T.商品コード,S.商品名 FROM 注文 AS T JOIN 商品 AS S ON T.商品コード = S.商品コード WHERE T.注文日 >= '2020-08-01' AND T.注文日 < '2020-09-01' ORDER BY T.商品コード`;
      console.table(expected);
    ...
    test("65:問題64では、すでに廃番となっている商品は抽出されなかった。廃番となった商品ももれなく一覧に記載されるよう、SQL文を変更する。なお、廃番商品の商品名には「廃番」と表示すること。", async () => {
      const expected = await prisma.$queryRaw`SELECT T.商品コード,COALESCE(S.商品名,'廃番') AS 商品名 FROM 注文 AS T LEFT JOIN 商品 AS S ON T.商品コード = S.商品コード WHERE T.注文日 >= '2020-08-01' AND T.注文日 < '2020-09-01' ORDER BY T.商品コード`;
      console.table(expected);
    ...
    test("68:注文番号「202104030010」について、注文日、注文番号、注文枝番、商品コード、商品名、単価、数量、注文金額を抽出する。注文金額は単価と数量より算出し、その総額からクーポン割引料を差し引いたものとする。また、商品が廃番になっている場合は、廃番商品テーブルから必要な情報を取得すること。", async () => {
      const expected = await prisma.$queryRaw`SELECT T.注文日,T.注文番号,T.注文枝番,T.商品コード,COALESCE(S.商品名,H.商品名) AS 商品名,COALESCE(S.単価,H.単価) AS 単価,T.数量 AS 数量,COALESCE(S.単価,H.単価) * T.数量 - COALESCE(T.クーポン割引料,0) AS 注文金額 FROM 注文 AS T LEFT JOIN 商品 AS S ON T.商品コード = S.商品コード LEFT JOIN 廃番商品 AS H ON T.商品コード = H.商品コード WHERE T.注文番号 = '202104030010'`;
      console.table(expected);
...
describe("RPGデータベース", () => {
  ...
  describe("第3章 操作する行の絞り込み", () => {
    ...
    test("6:パーティーテーブルから、IDが「C02」のデータを抽出する。", async () => {
      const expected = await prisma.$queryRaw`SELECT * FROM パーティ WHERE "ID" = 'C02'`;
      console.table(expected);

      const parties = await prisma.party.findUnique({
        where: { id: "C02" },
      });
      const result = [{
        ID: parties.id,
        名称: parties.name,
        職業コード: parties.professionCode,
        HP: parties.hp,
        MP: parties.mp,
        状態コード: parties.statusCode,
      }];
      console.table(result);
    ...
    test("7:パーティーテーブルのID「A01」のデータについて、HPを120に更新する。", async () => {
      await prisma.$queryRaw`UPDATE パーティ SET "HP" = 120 WHERE "ID" = 'A01'`;
      const expected = await prisma.$queryRaw`SELECT * FROM パーティ WHERE "ID" = 'A01'`;
      console.table(expected);
    ...
    test("10:イベントテーブルから、タイプが「特殊」でないデータについて、イベント番号、イベント名称、タイプの一覧を抽出する。", async () => {
      const expected = await prisma.$queryRaw`SELECT "イベント番号", "イベント名称", "タイプ" FROM イベント WHERE "タイプ" <> '3'`;
      console.table(expected);
    ...
    test("13:イベントテーブルから、別のイベントのクリアを前提としないイベントについて、イベント番号とイベント名称を抽出する。", async () => {
      const expected = await prisma.$queryRaw`SELECT "イベント番号", "イベント名称" FROM イベント WHERE "前提イベント番号" IS NULL`;
      console.table(expected);
    ...
    test("14:イベントテーブルから、次に発生するイベントが決められれているイベントについて、イベント番号、イベント名称、後続イベント番号を抽出する。", async () => {
      const expected = await prisma.$queryRaw`SELECT "イベント番号", "イベント名称", "後続イベント番号" FROM イベント WHERE "後続イベント番号" IS NOT NULL`;
      console.table(expected);
      ...
      const result = parties.map((p) => {
        return {
          イベント番号: p.eventNumber,
          イベント名称: p.eventName,
          後続イベント番号: p.followOnEventNumber,
        };
      });
      console.table(result);
    ...
    test("15:名称に「ミ」が含まれるパーティーテーブルのデータについて、状態コードを「眠り」に更新する。", async () => {
      await prisma.$queryRaw`UPDATE パーティ SET "状態コード" = '01' WHERE "名称" LIKE '%ミ%'`;
      const expected = await prisma.$queryRaw`SELECT "状態コード" FROM パーティ WHERE "名称" LIKE '%ミ%'`;
      console.table(expected);
    ...
    test("16:HPが120～160の範囲にあるパーティーテーブルのデータについて、ID、名称、HPの一覧を抽出する。ただし、記述条件は1つであること。", async () => {
      const expected = await prisma.$queryRaw`SELECT "ID", "名称", "HP" FROM パーティ WHERE "HP" BETWEEN 120 AND 160`;
      console.table(expected);
    ...
    test("17:職業が「勇者」「戦士」「武道家」のいずれかであるパーティテーブルのデータについて、名称と職業コードを抽出する。ただし、記述する条件式は1つであること。", async () => {
      const expected = await prisma.$queryRaw`SELECT "名称", "職業コード" FROM パーティ WHERE "職業コード" IN ('01', '10', '11')`;
      console.table(expected);
    ...
    test("18:状態コードが「異常なし」と「気絶」のどちらでもないパーティテーブルのデータについて、名称と状態コードを抽出する。ただし、記述する条件式は1つであること。", async () => {
      const expected = await prisma.$queryRaw`SELECT "名称", "状態コード" FROM パーティ WHERE "状態コード" NOT IN ('00', '09')`;
      console.table(expected);
    ...
    test("19:パーティテーブルから、HPとMPがともに100を超えているデータを抽出する。", async () => {
      const expected = await prisma.$queryRaw`SELECT * FROM パーティ WHERE "HP" > 100 AND "MP" > 100`;
      console.table(expected);
    ...
    test("20:パーティテーブルから、IDが「A」で始まり、職業コードの1文字目が「2」であるデータを抽出する。", async () => {
      const expected = await prisma.$queryRaw`SELECT * FROM パーティ WHERE "ID" LIKE 'A%' AND "職業コード" LIKE '2%'`;
      console.table(expected);
    ...
    test("21:イベントテーブルから、タイプが「強制」で、事前にクリアが必要なイベントかつ次に発生するイベントが設定されているデータを抽出する。", async () => {
      const expected = await prisma.$queryRaw`SELECT * FROM イベント WHERE "タイプ" = '1' AND "前提イベント番号" IS NOT NULL AND "後続イベント番号" IS NOT NULL`;
      console.table(expected);
  ...
  describe("第8章 複数テーブルの結合", () => {
    ...
    test("63:イベントテーブルから、タイプ「強制」のイベントについて、イベント番号とイベント名称、パーティーのクリア区分を抽出する。ただし、これまでに未着手のイベントは考慮しなくてよい", async () => {
      const expected = await prisma.$queryRaw`
            SELECT
              M.イベント番号,M.イベント名称,E.クリア区分
            FROM イベント M JOIN 経験イベント E ON M.イベント番号 = E.イベント番号
            WHERE M.タイプ = '1' 
      `;
      console.table(expected);
      ...
      const result = events.map((e) => {
        return {
          イベント番号: e.eventNumber,
          イベント名称: e.eventName,
          クリア区分: e.experienceEvent[0] ? e.experienceEvent[0].clearType : "未クリア",
        };
      }).sort((a, b) => {
        if (a.イベント番号 < b.イベント番号) {
          return -1;
        } else if (a.イベント番号 > b.イベント番号) {
          return 1;
        } else {
          return 0;
        }
    ...
    test("64:問題63では、着手していないイベントについては抽出されなかった。未着手のイベントについてももれなく抽出できるよう、SQL文を変更する。なお、クリアしていないイベントについては、クリア区分に「未クリア」と表示する。", async () => {
      const expected = await prisma.$queryRaw`
            SELECT
              M.イベント番号,M.イベント名称,COALESCE(E.クリア区分,'未クリア') AS クリア区分
            FROM イベント M LEFT JOIN 経験イベント E ON M.イベント番号 = E.イベント番号
            WHERE M.タイプ = '1'
      `;
      console.table(expected);
    ...
    test("65:このテーブルを使って、現在のパーティーに参加しているキャラクターの一覧を適切な列を用いて次の別名で、ID順に抽出する。", async () => {
      const expected = await prisma.$queryRaw`
            SELECT
              P."ID",P.名称 AS なまえ,S.コード名称 AS 職業,J.コード名称 AS 状態
            FROM パーティ P JOIN (SELECT コード値,コード名称 FROM コード WHERE コード種別 = '1') S ON P.職業コード = S.コード値
            JOIN (SELECT コード値,コード名称 FROM コード WHERE コード種別 = '2') J ON P.状態コード = J.コード値
            ORDER BY "ID"
      `;
      console.table(expected);
    ...
    test("66:パーティーテーブルから、現在のパーティーに参加しているキャラクターの一覧を次の形式で抽出する。職業はコードテーブルより日本語で表示する。また、現在のパーティーにいない職業についてももれなく一覧に記載し、名称の項目に「（仲間になっていない！）」と表示すること。", async () => {
      ...
      const expected = await prisma.$queryRaw`
            SELECT
              P."ID",COALESCE(P.名称,'（仲間になっていない！）') AS なまえ,S.コード名称 AS 職業
            FROM パーティ P RIGHT JOIN (SELECT コード値,コード名称 FROM コード WHERE コード種別 = '1') S ON P.職業コード = S.コード値
            ORDER BY "ID"
      `;
      console.table(expected);
      ...
      const result = profession.map((pro) => {
        const partyId = party.find((p) => p.professionCode === pro.value)?.id;
        const partyName = party.find((p) => p.professionCode === pro.value)?.name;
        return {
          "ID": partyId ? partyId : null,
    ...
    test("67:経験イベントテーブルから、参加済みイベントのクリア結果一覧を次の形式で抽出する。クリア結果は「コード値：コード名称」のように表示し、クリア未済のイベントも記載されるよう考慮する。", async () => {
      ...
      const clearTypes = await prisma.code.findMany({
        where: {
          type: "4",
        },
        select: {
      ...
      const result = events.map((e) => {
        const clearType = (experienceEvent) => {
          if (experienceEvent[0] === null) {
            return null;
          } else if (experienceEvent[0] === undefined) {
            return undefined;
          } else if (experienceEvent[0].clearType === null) {
            return null;
          } else {
            return experienceEvent[0].clearType;
          }
        }
        const clearResult = (experienceEvent) => {
          const clearTypeName = clearTypes.find((c) => c.value === e.experienceEvent[0]?.clearResult)?.name;

          if (experienceEvent[0] === null) {
            return null;
          } else if (experienceEvent[0] === undefined) {
            return undefined;
          } else if (experienceEvent[0].clearResult === null) {
            return null;
          } else {
            return `${experienceEvent[0].clearResult}:${clearTypeName}`;
          }
        }
        return {
          "イベント番号": e.eventNumber,
          "クリア区分": clearType(e.experienceEvent),
          "クリア結果": clearResult(e.experienceEvent),
        };
      }).filter((e) => e.クリア区分 !== undefined).sort((a, b) => {
        if (a.イベント番号 < b.イベント番号) {
          return -1;
        } else if (a.イベント番号 > b.イベント番号) {
          return 1;
    ...
    test("68:イベントテーブルから、前提イベントが設定されているイベントについて、次の形式の一覧を抽出する。", async () => {
      ...
      const expected = await prisma.$queryRaw`
            SELECT
              E1.イベント番号,E1.イベント名称,E1.前提イベント番号,E2.イベント名称 AS 前提イベント名称
            FROM イベント E1 JOIN イベント E2 ON E1.前提イベント番号 = E2.イベント番号
      ...
      const premiseEventNames = await prisma.event.findMany({
        select: {
          eventNumber: true,
          eventName: true,
        },
      ...
      const result = events.map((e) => {
        return {
          ...
          "イベント名称": e.eventName,
          "前提イベント番号": e.premiseEventNumber,
          "前提イベント名称": premiseEventNames.find((p) => p.eventNumber === e.premiseEventNumber)?.eventName,
        };
      });
      console.table(result);
    ...
    test("69:イベントテーブルから、前提イベントまたは後続イベントが設定されているイベントについて、次の形式の一覧を抽出する。", async () => {
      ...
      const expected = await prisma.$queryRaw`
            SELECT
              E1.イベント番号,E1.イベント名称,E1.前提イベント番号,E2.イベント名称 AS 前提イベント名称,E1.後続イベント番号,E3.イベント名称 AS 後続イベント名称
            FROM イベント E1
              LEFT JOIN イベント E2 ON E1.前提イベント番号 = E2.イベント番号
              LEFT JOIN イベント E3 ON E1.後続イベント番号 = E3.イベント番号
      ...
      const result = events.map((e) => {
        const eventName = (premiseEventNumber) => {
          const result = eventNames.filter((n) => n.eventNumber === premiseEventNumber);
          if (result.length === 0) {
            return null;
          } else {
        ...
        return {
          ...
          "イベント名称": e.eventName,
          "前提イベント番号": e.premiseEventNumber,
          "前提イベント名称": eventName(e.premiseEventNumber),
          "後続イベント番号": e.followOnEventNumber,
          "後続イベント名称": eventName(e.followOnEventNumber),
        };
      });
      console.table(result);
    ...
    test("70:ほかのイベントの前提となっているイベントについて、次の形式の一覧を抽出する。一覧はイベント番号順とする。", async () => {
      ...
      const expected = await prisma.$queryRaw`
            SELECT
              E.イベント番号,E.イベント名称,Z.前提イベント数
            FROM イベント E
              JOIN (SELECT 前提イベント番号,COUNT(前提イベント番号) AS 前提イベント数
                    FROM イベント
                    WHERE 前提イベント番号 IS NOT NULL GROUP BY 前提イベント番号) Z ON E.イベント番号 = Z.前提イベント番号
            ORDER BY "イベント番号"
      `;
      console.table(expected);
      ...
      const premiseEventCount = {};
      for (const event of events) {
        const premiseEventNumber = event._sum.premiseEventNumber;
        premiseEventCount[premiseEventNumber] = (premiseEventCount[premiseEventNumber] || 0) + 1;
      }

      const result = events.map((event) => ({
        "イベント番号": event.eventNumber,
        "イベント名称": event.eventName,
        "前提イベント数": premiseEventCount[event.eventNumber] || 0,
      }))
        .sort((a, b) => a.イベント番号 - b.イベント番号)
        .filter((e) => e.前提イベント数 > 0);
